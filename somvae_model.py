...
class SOMVAE(tf.keras.Model):
    ...
    def __init__(self, latent_dim=64, som_dim=[8,8], learning_rate=1e-4, decay_factor=0.95, decay_steps=1000,
        ...
        self.inputs = tf.Variable(tf.zeros(shape=[batch_size, input_length, input_channels, 1],dtype=tf.int32),shape=[32,28, 28, 1])
    ...
    def get_encoder(self):
        if not self.mnist:
            ...
            h_0 = tf.keras.layers.Input(shape=[self.input_length, self.input_channels,1], name='input',dtype=tf.int32)
    ...
    def get_decoder(self):
        ...
        if not self.mnist:
            ...
            h_0 = tf.keras.layers.Input(shape=self.latent_dim, name='latent_code',dtype=tf.int32)
            flat_size = 7*7*256
