...
class SOMVAE(tf.keras.Model):
    ...
    def get_encoder(self):
        if not self.mnist:
            ...
            h_0 = tf.keras.layers.Input(shape=[self.input_length, self.input_channels,1], name='input',dtype=tf.float64)
    ...
    def get_decoder(self):
        ...
        if not self.mnist:
            ...
            h_0 = tf.keras.layers.Input(shape=self.latent_dim, name='latent_code',dtype=tf.float64)
            flat_size = 7*7*256
